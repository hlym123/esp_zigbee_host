// https://github.com/espressif/esp-zigbee-sdk/blob/main/examples/esp_zigbee_host/components/src/esp_host_bus.c
...
{
    return (uart_write_bytes(g_uart_num, (const char *)buffer, size) == size) ? ESP_OK : ESP_ERR_INVALID_SIZE;
}
...
static void esp_host_bus_task(void *pvParameter)
{
    ...
    while (bus->state == BUS_INIT_START) {
        if (xQueueReceive(uart0_queue, (void *)&event, (TickType_t)portMAX_DELAY)) {
            ...
            switch (event.type) {
                case UART_DATA:
                    host_event.size = uart_read_bytes(g_uart_num, dtmp, event.size, portMAX_DELAY);
...
esp_err_t esp_host_bus_start(esp_host_bus_t *bus)
{
    ...
    return (xTaskCreate(esp_host_bus_task, "esp_host_bus_task", HOST_BUS_TASK_STACK, bus, HOST_BUS_TASK_PRIORITY,
                        NULL) == pdTRUE)
               ? ESP_OK
               : ESP_FAIL;
}
